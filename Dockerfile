...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y git wget unzip dos2unix
...
RUN wget https://sourceforge.net/projects/primer3/files/primer3/2.3.7/primer3-2.3.7.tar.gz -O primer3-2.3.7.tar.gz  && \
    ...
    sed -i -e 's,/opt/primer3_config,/usr/local/share/primer3/primer3_config,g' primer3-2.3.7/src/primer3_boulder_main.c && \
    make -C primer3-2.3.7/src && \
    install primer3-2.3.7/src/primer3_core /usr/local/bin/ && \
    install primer3-2.3.7/src/ntdpal /usr/local/bin/ && \
    install primer3-2.3.7/src/ntthal /usr/local/bin/ && \
    install primer3-2.3.7/src/oligotm /usr/local/bin/ && \
    install primer3-2.3.7/src/long_seq_tm_test /usr/local/bin/ && \
